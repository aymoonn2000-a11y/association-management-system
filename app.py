...
# 1. تهيئة إعدادات الصفحة (تُستدعى مرة واحدة فقط في أول سطر برامجي لـ Streamlit)
st.set_page_config(
...
def init_tables():
    ...
    ''')
    # جدول الموظفين و عهد البرامج الخيرية
    run_db_query('''
    ...
    ''')
    # جدول حركة التوصيل والمراسلات الخارجية
    run_db_query('''
...
with tab3:
    ...
    with col_i3: i_min = st.number_input("حد الأمان لتنبيه الإدارة قبل النفاد", min_value=1, value=5)
    with col_i4: i_direct = st.number_input("صادر مباشر بدون عهدة (تالف/استهلاك مباشر)", min_value=0, value=0)
    ...
    if not df_inv.empty:
        ...
        # تصدير ملف إكسيل حقيقي متعدد الصفحات
        try:
            output = BytesIO()
            ...
            st.download_button(
                label="📥 تحميل التقرير الشامل لنظام الجمعية كملف Excel",
                data=processed_data,
...
with tab4:
    ...
    with col_g2:
        if not df_emp.empty and 'program' in df_emp.columns:
            st.write("📈 **توزيع استهلاك الموارد والمستلزمات حسب برامج الجمعية**")
